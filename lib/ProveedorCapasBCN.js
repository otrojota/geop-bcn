...
class ProveedorCapasBCN extends ProveedorCapas {
    constructor(opciones) {
        ...
        this.addOrigen("ine", "Instituto Nacional de Estadísticas", "https://www.ine.cl/", "./img/ine.png");
       ...
       let capaRegiones = new CapaObjetosConDatos("bcn", "Regiones", "Regiones", "bcn", {
        ...
        let capaRedHidrografica = new CapaObjetosConDatos("bcn", "RedHidrografica", "Red Hidrográfica", "bcn", {
            temporal:false,
            datosDinamicos:false,
            menuEstaciones:false,
            geoJSON:true,
            estilos:(function(f) {return {stroke:"#0000ff", strokeWidth:2}}).toString()
        }, [], "img/rio.svg");
    ...
    getFeaturesProvincias() {
        ...
        features.features.forEach(f => {
            f.properties.id = f.properties.cod_prov;
    ...
    getFeaturesComunas() {        
        ...
        features.features.forEach(f => {
            ...
            f.properties.codigoRegion = this.pad(f.properties.codregion, 2);
            f.properties._codigoDimension = this.pad(f.properties.cod_comuna, 5);
            let codregion = f.properties.codregion;
            if (filtrosRegion[codregion] === undefined) {
                filtrosRegion[codregion] = 0;
            }
    ...
    getFeaturesRedHidrografica() {
        ...
        let fProvincia = Object.keys(filtrosProvincia).map(provincia => ({
            nombre:provincia + " [" + filtrosProvincia[provincia] + "]",
            filtro:`function(f) {return f.properties.Provincia == "${provincia}"}`,
            activo:provincia == "Valparaíso"
        }));
        fProvincia.sort((f1, f2) => (f1.nombre > f2.nombre?1:-1));
        features._filtros = [
            {titulo:"Filtrar por Provincia", filtros:fProvincia}
        ]
